...
**Output format.** The pipeline produces HL7 FHIR R4 Bundles containing: `Encounter` (visit context), `Composition` (SOAP note sections as XHTML narrative with LOINC section codes -- 11488-4 for Consultation Note), `DiagnosticReport` (imaging findings), `Condition` (one resource per ICD-10 code with canonical system URI `http://hl7.org/fhir/sid/icd-10-cm`), and `MedicationStatement` (extracted prescriptions with RxNorm codes where extractable). All generated bundles pass structural validation against the HL7 FHIR R4 specification. The audit trail `Provenance` resource records model version, inference timestamp, and agent execution chain for regulatory traceability. The architecture is designed for EHR integration via SMART on FHIR authorisation, enabling launch from within Epic or Cerner without data leaving the EHR environment.
